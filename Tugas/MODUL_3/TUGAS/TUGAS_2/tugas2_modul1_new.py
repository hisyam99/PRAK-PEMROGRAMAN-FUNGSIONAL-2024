...
# list comprehension
def daftar_teman_2(state, nim):
    return [t for t in state['teman'].get(nim, [])]

# map
def ubah_judul_ke_upper(buku):
    return buku['judul'].upper()
def judul_buku_upper(state):
    return list(map(ubah_judul_ke_upper, state['buku'].values()))

# filter
def is_buku_habis(buku):
    return buku['jumlah'] == 0
def buku_habis(state):
    return list(filter(is_buku_habis, state['buku'].values()))

# reduce
def jumlah_buku(total, buku):
    return total + buku['jumlah']
def total_buku(state):
    return reduce(jumlah_buku, state['buku'].values(), 0)

# rekursif
def hitung_total_buku(buku_list, index=0):
    if index >= len(buku_list):
        return 0
    else:
        return buku_list[index]['jumlah'] + hitung_total_buku(buku_list, index + 1)
            ...
            if user_nim:
                ...
                if state['profil'][user_nim]['role'] == 'admin':
                    ...
                    while True:
                        ...
                        print("10. Keluar")
                        print("11. daftar teman(list comprehension)")
                        print("12. list buku dalam huruf kapital(map)")
                        print("13. list buku habis(filter)")
                        print("14. total buku keseluruhan yang tersedia(reduce)")
                        print("15. total buku keseluruhan yang tersedia(rekursif)")
                        ...
                        if pilihan_user == '1':
                            ...
                            teman = daftar_teman_2(state, user_nim)
                            for t in teman:
                                print(t)
                        elif pilihan_user == '12':
                            judul_buku_uppercase = judul_buku_upper(state)
                            print("Judul Buku dalam Huruf Kapital:")
                            for judul in judul_buku_uppercase:
                                print(judul)
                        elif pilihan_user == '13':
                            buku_habis_list = buku_habis(state)
                            print("Daftar Buku yang Stoknya Habis:")
                            for buku in buku_habis_list:
                                print(buku['judul'])
                        elif pilihan_user == '14':
                            total_buku_di_perpustakaan = total_buku(state)
                            print(f"Total Buku di Perpustakaan: {total_buku_di_perpustakaan}")
                        elif pilihan_user == '15':
                            total_bukus = hitung_total_buku(list(state['buku'].values()))
                            print(f"Total Buku di Perpustakaan: {total_bukus}")
                        else:
                            print("Pilihan tidak valid. Silakan coba lagi.")
