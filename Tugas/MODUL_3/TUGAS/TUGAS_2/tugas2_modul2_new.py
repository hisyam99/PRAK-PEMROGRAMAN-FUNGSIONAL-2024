...
# list comprehension
def daftar_produk_dan_pendapatan(data):
    return [(produk["nama_produk"], produk["pendapatan"]) for produk in data]

# map
def to_upper(produk):
    return produk["nama_produk"].upper()
def daftar_produk_upper(data):
    return list(map(to_upper, data))

#filter
def pendapatan_lebih_dari_100k(produk):
    return produk["pendapatan"] >= 500000
def produk_berpendapatan_tinggi(data):
    return list(filter(pendapatan_lebih_dari_100k, data))

#reduce
def jumlahkan_pendapatan(total, produk):
    return total + produk["pendapatan"]
def total_pendapatan(data):
    return reduce(jumlahkan_pendapatan, data, 0)

#rekursif
def total_quantity(data, index=0):
    if index >= len(data):
        return 0
    return data[index]["quantity"] + total_quantity(data, index + 1)
...
def main_menu():
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            data_dengan_pendapatan = hitung_pendapatan(data_penjualan)
            daftar_produk = daftar_produk_dan_pendapatan(data_dengan_pendapatan)
            print("Daftar Produk dan Pendapatan:")
            for produk, pendapatan in daftar_produk:
                print(f"{produk}: Rp{pendapatan}")
        elif pilihan == "6":
            daftar_produk_kapital = daftar_produk_upper(data_penjualan)
            print("Nama Produk dalam Huruf Kapital:")
            for produk in daftar_produk_kapital:
                print(produk)
        elif pilihan == "7":
            produk_tinggi = produk_berpendapatan_tinggi(data_dengan_pendapatan)
            print("Produk dengan Pendapatan di Atas Rp500,000:")
            for produk in produk_tinggi:
                print(f"{produk['nama_produk']}: Rp{produk['pendapatan']}")
        elif pilihan == "8":
            data_dengan_pendapatan = hitung_pendapatan(data_penjualan)
            total = total_pendapatan(data_dengan_pendapatan)
            print(f"Total Pendapatan Semua Produk: Rp{total}")
        elif pilihan == "9":
            total = total_quantity(data_penjualan)
            print(f"Total Quantity Semua Produk: {total}")
        else:
